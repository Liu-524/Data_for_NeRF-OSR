...
env_dir_15 = movie_dir / 'envmaps-15'
env_dir_30 = movie_dir / 'envmaps-30'
intrinsics_dir = movie_dir / 'intrinsics'
pose_dir = movie_dir / 'pose'
rgb_dir = movie_dir / 'rgb'
env_dir_0.mkdir(exist_ok=True)
env_dir_15.mkdir(exist_ok=True)
env_dir_30.mkdir(exist_ok=True)
...
for i in range(end_frame - start_frame + 1):
    ...
    dump_env(env_dir_0 / '{:0>6d}.txt'.format(i), env)
    dump_env(env_dir_15 / '{:0>6d}.txt'.format(i), R_15 @ env)
    dump_env(env_dir_30 / '{:0>6d}.txt'.format(i), R_30 @ env)
    shutil.copy(base_dir / 'train/rgb/0_{:0>10d}.png'.format(i + start_frame), rgb_dir / "{:0>6d}.png".format(i))
    shutil.copy(base_dir / 'train/intrinsics/0_{:0>10d}.txt'.format(i + start_frame), intrinsics_dir / "{:0>6d}.txt".format(i))
    shutil.copy(base_dir / 'train/pose/0_{:0>10d}.txt'.format(i + start_frame), pose_dir / "{:0>6d}.txt".format(i))
