...
class HighbayDataset(BaseDataset):
    def __init__(self, root_dir, split, nvs=False, downsample=1.0, load_2d=True, generate_render_path=False, **kwargs):
        ...
        K[:2] *= downsample
        self.K = K
        w, h = int(intrinsic['w']*downsample), int(intrinsic['h']*downsample)
        self.img_wh = (w, h)
        ...
        valid_ids, valid_time  = self.get_valid_time(img_time, frame_start, frame_end)
        self.setup_poses(sensor_data, valid_time)
        print('#frames = {}'.format(len(valid_time)))

        if split != 'train':
            sample = np.arange(0, 100, 10)
            self.poses = self.poses[sample]
            valid_ids = valid_ids[sample]
        ...
        if load_2d:
            ...
            if self.split == 'train':
                ...
                print('Load Shadow ...')
                shadow_0 = self.read_shadow(dir_shadow_0, valid_ids)
                shadow_1 = self.read_shadow(dir_shadow_1, valid_ids)
                self.shadows = torch.FloatTensor(np.concatenate([shadow_0, shadow_1], axis=0))
                print('Load Deshadow ...')
                deshadow_0 = self.read_rgb(dir_deshadow_0, valid_ids)
                deshadow_1 = self.read_rgb(dir_deshadow_1, valid_ids)
                self.rgb_deshadow = torch.FloatTensor(np.concatenate([deshadow_0, deshadow_1], axis=0))

    def get_valid_time(self, img_time, frame_start, frame_end):
        valid_ids = np.arange(len(img_time))
        valid_ids = valid_ids[frame_start:frame_end]
        valid_time = img_time[frame_start:frame_end]
    ...
    def setup_poses(self, sensor_data, valid_time):
        ...
        lla = np.stack([latitude, longitude, height])
        f_lla = interp1d(sensor_time, lla)
        lla_sample = f_lla(valid_time).T
        self.estimate_sunlight(lla_sample, valid_time)
    ...
    def estimate_sunlight(self, lat_lon_alt, valid_time):
        ...
        for i in range(len(valid_time)):
            ...
            sz = np.cos(theta)                # up
            sy = np.sin(theta) * np.cos(phi)  # north
            sx = np.sin(theta) * np.sin(phi)  # east
        ...
        self.up_dir = torch.FloatTensor((0, 0, 1))
    ...
    def read_shadow(self, dir_shadow, valid_ids):
        shadow_list = []
        for i in valid_ids:
            path = os.path.join(dir_shadow, '{:0>5d}.png'.format(i))
